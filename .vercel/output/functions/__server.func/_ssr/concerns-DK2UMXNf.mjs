...
import { t as ageFromDob } from "./format-DK-owqV6.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/concerns-DK2UMXNf.js
function buildConcerns(pq, totals = computeTotals(pq)) {
	const age = ageFromDob(pq.client.dob) ?? ageFromDob(pq.spouse.dob);
	const withdrawal = totals.investableAssets > 0 && totals.shortage < 0 ? Math.abs(totals.shortage) / totals.investableAssets * 100 : 0;
	const qualifiedShare = totals.totalAssetsExRE > 0 ? totals.deferred / totals.totalAssetsExRE : 0;
	const rothShare = totals.totalAssetsExRE > 0 ? totals.roth / totals.totalAssetsExRE : 0;
	totals.totalAssetsExRE > 0 && totals.cash / totals.totalAssetsExRE;
	...
	const namedBeneficiaries = [
		...
		...pq.investments,
		...pq.insurance
	].filter((r) => "beneficiary" in r ? Boolean(r.beneficiary?.trim()) : false).length;
	const accountCount = pq.deferred.length + pq.roth.length + pq.investments.length + pq.insurance.length;
	const advisorNamed = [
	...
	return [
		{
			...
			summary: totals.shortage < 0 ? `Guaranteed income covers ${formatCover(totals)} of today’s ${fmt(totals.expenses)} spending, leaving a ${fmt(Math.abs(totals.shortage))} annual gap to fund from investments${withdrawal ? ` (~${withdrawal.toFixed(1)}% withdrawal).` : "."}` : `Guaranteed income of ${fmt(totals.guaranteedRetirement)} covers today’s ${fmt(totals.expenses)} spending, with a ${fmt(totals.shortage)} surplus before tax and COLA.`,
			strategy: [
		...
		},
		{
			id: "tax",
			title: "Tax planning",
			severity: qualifiedShare > .4 && rothShare < .05 ? "high" : "watch",
			applies: totals.deferred > 0 || totals.investments > 0,
			summary: rothShare < .02 ? `The portfolio is not tax-diversified. ${fmt(totals.deferred)} sits in tax-deferred accounts and ${fmt(totals.roth)} in Roth — most withdrawals will be ordinary income.` : `Tax location is mixed: deferred ${fmt(totals.deferred)}, taxable ${fmt(totals.investments)}, Roth ${fmt(totals.roth)}. Active management can still reduce lifetime tax.`,
			strategy: [
		...
		},
		{
			...
			summary: "There is currently no Roth / tax-free balance. Future rate increases and RMDs will push ordinary income into higher brackets with little flexibility.",
			strategy: [
		...
		},
		{
			...
			severity: totals.weightedFeePct >= .5 ? "high" : "info",
		...
		},
		{
			...
			summary: age !== null ? `At age ${age}, RMDs from ${fmt(totals.deferred)} of qualified assets will begin the year after ${age >= 73 ? "they started" : "age 73"} and are taxed as ordinary income.` : `Qualified balances of ${fmt(totals.deferred)} will generate RMDs beginning at 73 (75 if born 1960+).`,
			strategy: [
		...
		},
		{
			...
			applies: true,
			summary: "Premature death, long-term care, and liability claims can undo an otherwise funded plan. Coverage should be sized to cash-flow, not to a product.",
			strategy: [
		...
		},
		{
			...
			applies: !pq.hasLtc,
			summary: "No long-term care policy is on file. Private-room nursing costs now routinely exceed $100,000 per year and are not covered by Medicare beyond a short skilled-care window.",
			strategy: ["Compare traditional LTC, hybrid life/LTC, and a dedicated self-insurance reserve.", "Document the family-care preference so the surviving spouse is not the default plan."]
		},
		{
			id: "umbrella",
			title: "Umbrella liability protection",
			severity: pq.hasUmbrella ? "info" : "watch",
			applies: !pq.hasUmbrella && totals.netWorth > 1e6,
			...
			strategy: ["Add an umbrella of at least $1–5 million, coordinated with underlying auto and home limits.", "Confirm the umbrella sits over all licensed drivers and properties."]
		},
		{
			id: "estate",
		...
		},
		{
			...
			summary: "Trusts, TOD/POD, and joint titling each transfer differently. A trust that is not on the account title still goes through probate.",
			strategy: ["Audit every non-qualified account and deed against the trust.", "Use TOD/POD only as a complement to, not a substitute for, a funded trust on larger estates."]
		},
		{
			id: "beneficiaries",
		...
		},
		{
			...
			severity: "info",
			applies: true,
			summary: "Funeral, medical, and administrative costs arrive before accounts retitle. Social Security’s burial benefit is $255 against a typical $15,000 funeral.",
			strategy: ["Keep a dedicated cash reserve for final expenses, legal bills, and unpaid debts.", "Or fund a small final-expense policy so the family is not waiting on probate."]
		},
		{
			id: "coordination",
			...
			applies: true,
			summary: advisorNamed < 2 ? "The CPA, estate attorney, and insurance agent are not fully on file. Uncoordinated advice is how tax, titling, and insurance recommendations collide." : "Advisors are named. The next step is a working cadence so tax, estate, and investment moves happen in one sequence.",
			strategy: ["Introduce the CFP®, CPA, and estate attorney on a shared planning memo.", "Run Roth conversions, gifting, and trust funding as one calendar, not three."]
		}
	].filter((c) => c.applies);
}
function fmt(n) {
...
var ASSET_CLASSES = {
	...
	fixed: [
		...
		"US Treasuries",
		"Private Debt",
		"International ST Bonds"
	]
};
//#endregion
export { buildConcerns as i, INVESTMENT_STRATEGIES as n, TAX_STRATEGIES as r, ASSET_CLASSES as t };
