...
async function createPgliteSql(): Promise<Sql> {
  ...
  // Persist to disk in the workspace so discovery meetings survive a process
  // restart. Serverless (Vercel) stays in-memory — Neon is the store there.
  // New app tables (e.g. household_documents) apply from migrations/*.sql.
  globalRef.__pgliteInstance__ ??= (async () => {
    ...
    if (!process.env.VERCEL) {
      try {
        ...
        dataDir = join(process.cwd(), "data", "pglite");
    ...
    if (dataDir) {
      try {
        ...
        console.error("[db] PGLite file store failed, using memory:", err);
