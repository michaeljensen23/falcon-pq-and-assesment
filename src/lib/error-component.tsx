...
import { publicError } from "@/lib/intake/limits";
...
export function AppErrorComponent({ error }: ErrorComponentProps) {
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-3 bg-paper px-6 text-center text-ink">
      ...
      <p className="max-w-md text-sm break-words text-slate">
        {publicError(error, "An unexpected error occurred. Try reloading the page.")}
      </p>
    </main>
  );
}