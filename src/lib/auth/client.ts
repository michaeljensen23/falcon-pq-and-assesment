...
/**
 * True when this page cannot complete a top-level OAuth redirect — the live
 * preview iframe (`*.grok-sandbox.com` or any embed). Full-page navigation to
 * Google/the broker is refused inside an iframe (X-Frame-Options / 403), so
 * sign-in must open a popup instead.
 */
...
/** Named browsing context for the live-preview Google/X window (must match the login <a target>). */
export function oauthPopupTarget(providerId: string): string {
  return `grok-signin-${providerId}`;
}

export function oauthPopupPath(providerId: string): string {
...
export async function signIn(
  ...
  if (inLivePreview()) {
    // Native <a target> on the login button may open the window even when
    // window.open is blocked. Grab that named context after the click default.
    let handle = popup;
    if (!handle) {
      await new Promise((r) => window.setTimeout(r, 0));
      try {
        handle = window.open("", oauthPopupTarget(providerId));
        if (handle) {
          try {
            ...
            // Cross-origin already (broker/Google) — keep it.
          }
        }
      } catch {
        handle = null;
    ...
    if (!handle) throw new Error("Pop-up blocked — allow pop-ups for sign-in");
    const token = await waitForPopupToken(handle);
...
function openSignInPopup(providerId: string): Window | null {
  ...
  const name = oauthPopupTarget(providerId);
  // Open as a named tab (no `popup,width=…` features). Sized popups are treated
  // as pop-ups by iframe blockers and often return null on the first click;
  // a featureless window.open is the same user-gesture navigation as target=_blank.
  try {
 ...
 */
function waitForPopupToken(popup: Window | null): Promise<string | null> {
  return new Promise((resolve) => {
    ...
    if (popup) {
      ...
      // Native <a> opened the window but we have no handle — wait for
      // postMessage, then give up so the login button is not stuck forever.
      abandonTimer = window.setTimeout(() => settle(null), 90_000);
    }
    function cleanup() {
      if (pollTimer !== undefined) window.clearInterval(pollTimer);
      if (closeTimer !== undefined) window.clearTimeout(closeTimer);
      if (abandonTimer !== undefined) window.clearTimeout(abandonTimer);
